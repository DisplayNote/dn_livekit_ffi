...
use std::{
    env, error::Error, fs::{self, File}, io::{self, BufRead, Write}, path, process::Command, thread, time::Duration
};
...
pub const WEBRTC_PREFIXED_RELEASE: &str = "m114.5735.09";
...
pub fn merge_webrtc_lib() -> Result<(), Box<dyn Error>> {
    ...
    let flag_file = dir.join("webrtc_lib_merged.flag");

    // Check if the flag file exists
    if flag_file.exists() {
        println!("merge_webrtc_lib() has already been executed. Skipping.");
        return Ok(());
    }
